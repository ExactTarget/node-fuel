...
fuel._performTokenRequest = memoize(fuel._performRequest, function (options) {
	...
	return (error) ? 10000 : (body.expiresIn - 30) * 1000;
});
...
function deepExtend(dest, source) {
	for (var prop in source) {
		if (Object.prototype.toString.call(source[prop]) === "[object Array]") {
			dest[prop] = dest[prop] || [];
			for (var i = 0; i < source[prop].length; i += 1) {
				if (typeof source[prop][i] === 'object') {
					dest[prop][i] = deepExtend({}, source[prop][i]);
				} else {
					dest[prop][i] = source[prop][i];
				}
			}
...
function createConfigurationWrapper(func) {
	return function (storedOptions) {
		return function (options, callback) {
			...
			return func(deepExtend(deepExtend({}, storedOptions), options), callback);
		};
	};
}
